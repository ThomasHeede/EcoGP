...
class Inputs:
    ...
    def read_config(self, config_file: str):
        ...
        assert not self.coords_path, "Coordinates cannot be specified in current version (traits x gamma)."
...
def train(inputs: Inputs):
    ...
    # TODO: rewrite when tested! Splitting of species into different sets
    # N_SPECIES = dataset.n_species
    species_splits = random_split(torch.randperm(dataset.n_species), inputs.split_pct)
    train_species       = species_splits[0].indices
    validation_species  = species_splits[1].indices
    test_species        = species_splits[2].indices
    ...
    if inputs.save_model_path:
        ...
        if inputs.x_path:
            ...
            if inputs.traits_path:
                ...
                pd.DataFrame(w_loc, columns=dataset.taxon_names[train_species]).to_csv(os.path.join(inputs.save_model_path, "weights_traits_env_w.csv"))
                ...
                pd.DataFrame(w_loc, columns=dataset.taxon_names[train_species]).to_csv(os.path.join(inputs.save_model_path, "weights_environmental_w.csv"))
    ...
    for idx in validation_dataloader:
        ...
        Y = Y[:, validation_species]
        traits = traits[validation_species, :]
    ...
    pd.DataFrame(prob, columns=dataset.taxon_names[validation_species], index=dataset.site_names[validation_dataset.indices]).to_csv(os.path.join(inputs.save_model_path, "Y_pred_validation.csv"))
    pd.DataFrame(validation_Y, columns=dataset.taxon_names[validation_species], index=dataset.site_names[validation_dataset.indices]).to_csv(os.path.join(inputs.save_model_path, "Y_true_validation.csv"))
    ...
    if inputs.save_model_path:
        ...
        pd.DataFrame(metrics_per_species, columns=metrics_per_species.keys(), index=dataset.taxon_names[validation_species]).to_csv(os.path.join(inputs.save_model_path, "metrics_per_taxon_validation.csv"))
    ...
    pd.DataFrame(prob, columns=dataset.taxon_names[test_species], index=dataset.site_names[test_dataset.indices]).to_csv(os.path.join(inputs.save_model_path, "Y_pred_test.csv"))
    pd.DataFrame(test_Y, columns=dataset.taxon_names[test_species], index=dataset.site_names[test_dataset.indices]).to_csv(os.path.join(inputs.save_model_path, "Y_true_test.csv"))

    metrics_per_species = calculate_metrics(test_Y, prob)
    if inputs.save_model_path:
        import pandas as pd

        pd.DataFrame(metrics_per_species, columns=metrics_per_species.keys(), index=dataset.taxon_names[test_species]).to_csv(os.path.join(inputs.save_model_path, "metrics_per_taxon_test.csv"))
